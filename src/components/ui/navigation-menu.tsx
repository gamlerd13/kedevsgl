...
>(({ className, ...props }, ref) => (
  <div
    // className={cn("absolute bg-indigo-600 left-0 top-full flex justify-center")}
    className={cn(
      "absolute left-1/2 transform top-[100%] -translate-x-1/2 flex justify-center",
    )}
  >
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Indicator
    ref={ref}
    className={cn(
      "top-0 z-[100] flex h-1.5 items-end justify-center overflow-hidden data-[state=visible]:animate-in data-[state=hidden]:animate-out data-[state=hidden]:fade-out data-[state=visible]:fade-in bg-indigo-600",

      className,
    )}
    {...props}
  >
    <div className="relative h-[50px] w-[50px] rotate-45 rounded-tl-sm bg-indigo-600 shadow-md dark:bg-red-800" />
  </NavigationMenuPrimitive.Indicator>
));
